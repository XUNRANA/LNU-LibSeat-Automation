...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Manual annotation tool for click1 captcha dataset.")
    parser.add_argument(
        "--dataset",
        type=Path,
        default=Path("dataset/click1"),
    ...
    parser.add_argument(
        ...
        type=Path,
        default=None,
        help="CSV to import when --import-eval is used. Defaults to dataset/click1/eval_p0_ddddocr_failures/annotations_by_sample.csv.",
    )
    parser.add_argument(
        "--overwrite-labels",
        action="store_true",
...
def load_label(sample: Sample) -> dict[str, Any]:
    ...
    return {
        "target_count": int(data.get("target_count") or max(len(points), 1) or 1),
...
class AnnotatorApp:
    def __init__(
        ...
        self.root.title("click1 manual annotator")
    ...
    def bind_keys(self) -> None:
        ...
        self.root.bind("4", lambda _event: self.set_target_count(4))
    ...
    def load_current(self) -> None:
        sample = self.sample
        data = load_label(sample)
        self.target_count = min(max(int(data["target_count"]), 1), 4)
    ...
    def redraw(self) -> None:
        ...
        self.footer.config(
            text=(
                "左键添加点 | 右键/Z/Backspace撤销 | 1/2/3/4设置目标数 | "
