...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Build a 3-plan x 4-YOLO-candidate Siamese dataset from click3 raw samples."
    )
    parser.add_argument("--raw-dataset", type=Path, default=Path("dataset/click3"))
    parser.add_argument(
        "--split-images",
        type=Path,
        default=Path("dataset/click3_yolo_plan_bg_char_60_900_100/images/val"),
        help="Images whose stems define the sample list, e.g. YOLO val/test split.",
    )
    parser.add_argument("--split-name", default="val")
    parser.add_argument("--output", type=Path, default=Path("dataset/click3_siamese_yolo4_val"))
...
def load_raw_sample(raw_dataset: Path, sample_name: str) -> tuple[np.ndarray, np.ndarray, list[tuple[int, int]], dict[str, Any]]:
    ...
    if int(label.get("target_count", 0)) != 3 or len(label.get("points", [])) < 3:
        raise ValueError(f"not a 3-target sample: {sample_name}")
...
def extract_plan_crops(target_bgr: np.ndarray, args: argparse.Namespace) -> list[np.ndarray]:
    ...
    for x1, x2 in regions:
        crops.append(
            crop_with_padding(
                target_bgr,
                x1 - args.plan_padding,
                args.plan_y1 - args.plan_padding,
                x2 + args.plan_padding,
                args.plan_y2 + args.plan_padding,
                fill=(255, 255, 255),
            )
        )
    return crops
...
def compose_plan_bg(plan_crops: list[np.ndarray], bg_bgr: np.ndarray, plan_slot_height: int) -> np.ndarray:
...
def match_candidates_to_gt(
    ...
    """Return candidate_index -> target_index(1..3) and candidate_index -> nearest distance."""
    from itertools import permutations

    nearest_dist: dict[int, float] = {}
    for ci, cand in enumerate(candidates):
        nearest_dist[ci] = min(math.hypot(cand.center_x - gx, cand.center_y - gy) for gx, gy in gt_points)

    if len(candidates) < len(gt_points):
        return {}, nearest_dist

    best_score = float("inf")
    best_assignment: dict[int, int] = {}
    for cand_indices in permutations(range(len(candidates)), len(gt_points)):
        total = 0.0
        ok = True
        assignment: dict[int, int] = {}
        for target_index, cand_index in enumerate(cand_indices, start=1):
            gx, gy = gt_points[target_index - 1]
            dist = math.hypot(candidates[cand_index].center_x - gx, candidates[cand_index].center_y - gy)
            if dist > max_distance:
                ok = False
                break
            total += dist
            assignment[cand_index] = target_index
        if ok and total < best_score:
            best_score = total
            best_assignment = assignment
    return best_assignment, nearest_dist
...
def draw_preview(
    ...
    font = load_font(13)
    for index, (x, y) in enumerate(gt_points, start=1):
    ...
    for ci, cand in enumerate(candidates, start=1):
        target_index = assignment.get(ci - 1, 0)
        color = (0, 220, 132) if target_index else (255, 176, 0)
        draw.rectangle([cand.x1, cand.y1 - plan_slot_height, cand.x2, cand.y2 - plan_slot_height], outline=color, width=2)
        label = f"C{ci}->P{target_index}" if target_index else f"C{ci}->D"
        draw.text(
...
def write_dataset_info(output: Path, args: argparse.Namespace) -> None:
    info = {
        "format": "click3_siamese_yolo4",
        "description": "Each sample has plan001-003 and char001-004. meta.json labels char_target_index 1..3, or 0 for distractor.",
...
def main() -> None:
    args = parse_args()
    if args.top_k < 4:
        raise SystemExit("--top-k should be at least 4 for a 3x4 Siamese dataset")
    ...
    for sample_pos, sample_name in enumerate(sample_names):
        try:
            ...
            composed = compose_plan_bg(plan_crops, bg_bgr, args.plan_slot_height)
            candidates = detect_candidates(model, composed, args)
            assignment, nearest_dist = match_candidates_to_gt(candidates, gt_points, args.match_distance)

            matched_targets = sorted(set(assignment.values()))
            is_complete = len(candidates) >= args.top_k and matched_targets == [1, 2, 3]
            if not is_complete:
                incomplete += 1
                    ...
                    manifest_rows.append(
                        {
                            ...
                            "matched_targets": ",".join(str(value) for value in matched_targets),
                        }
                    )
                    continue
            ...
            for index, crop in enumerate(plan_crops, start=1):
                cv2.imwrite(str(out_dir / f"plan{index:03d}.png"), crop)
            ...
            for index, cand in enumerate(candidates[: args.top_k], start=1):
                ...
                target_index = assignment.get(index - 1, 0)
                char_item = {
                    "file": filename,
                    "char_index": index,
                    ...
                    "nearest_gt_distance": nearest_dist.get(index - 1),
                }
            ...
            for plan_index in range(1, 4):
                for char_item in char_items:
                    label = int(char_item["char_target_index"] == plan_index)
                    pair = {
                        "sample": sample_name,
                        "split": args.split_name,
                        "plan_index": plan_index,
                        "char_index": char_item["char_index"],
                        "plan_file": f"{sample_name}/plan{plan_index:03d}.png",
            ...
            meta = {
                ...
                "target": {
                    "plan_files": [f"plan{index:03d}.png" for index in range(1, 4)],
                    "plan_crop": {
                        "x1": args.plan_x1,
                        "x2": args.plan_x2,
                        "x3": args.plan_x3,
                ...
                "gt_points": [{"target_index": i + 1, "x": x, "y": y} for i, (x, y) in enumerate(gt_points)],
                "yolo": {
                    "model": str(args.yolo),
                    "imgsz": args.imgsz,
                    "conf": args.conf,
    ...
    with (output / "manifest.csv").open("w", newline="", encoding="utf-8") as fh:
        fieldnames = ["sample", "split", "status", "candidate_count", "matched_targets"]
