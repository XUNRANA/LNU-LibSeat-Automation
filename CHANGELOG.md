...
- `pyproject.toml`：工具配置单一来源（集中 ruff / pytest / mypy；含 `smoke` marker），取代分散的 `pytest.ini` 与 `mypy.ini`。
...
- 全仓类型标注收敛：建立 mypy 类型检查、告警 42 → 0（配置见 `pyproject.toml`，CI 以非阻塞报告形式持续收敛）。
