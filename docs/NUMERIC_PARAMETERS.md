...
| `HEARTBEAT_INTERVAL` | `1800` 秒 | main.py:113 | 长等待时心跳日志间隔（30 分钟） |
| 精确等待阈值 | `0.02` 秒 | main.py:143 | 剩余 <20ms 时切换到忙等待 |
| 忙等待 sleep | `0.01` 秒 | main.py:145 | 最后 20ms-500ms 阶段的 sleep 间隔 |
| 开火延迟 | `2` 秒 | main.py:337 | **关键**：到达开火时间后额外等 2 秒，确保服务器已切换状态 |
| 账号启动错开 | `8` 秒/账号 | main.py:670 | prep_at 偏移（非线程启动间隔），避免浏览器并发初始化 |
| `MAX_ACCOUNTS` | `2` | main.py:732 | 默认最大并发账号数 |
| 线程轮询间隔 | `0.5` 秒 | main.py:771 | 主线程等待子线程的轮询间隔 |
| 线程 join 超时 | `5` 秒 | main.py:780 | 关闭时等待线程退出的超时 |
...
| 参数 | 值 | 位置 | 说明 |
|------|---|------|------|
| 默认 WebDriverWait | `5` 秒 | booker.py:170 | SeatBooker 实例的通用元素等待超时 |
| 座位查找超时 | `5` 秒 | booker.py (XPath wait) | 等待座位元素出现的超时 |
| 弹窗轮询超时 | `3` 秒 | booker.py:395 | 点击座位后等待预约弹窗出现的最长时间 |
| 弹窗轮询间隔 | `0.1` 秒 | booker.py:416 | 每 100ms 检查一次弹窗 |
| 关弹窗后等待 | `0.1` 秒 | booker.py:358 | 关闭拦截弹窗后等 100ms 再重试点击 |
| 开始时间点击超时 | `1.5` 秒 | booker.py:430 | 点击开始时间标签的超时（fail-fast） |
| 时间轴渲染等待 | `0.3` 秒 | booker.py:439 | 选完开始时间后等 300ms，确保右侧时间轴渲染 |
| 结束时间点击超时 | `1.5` 秒 | booker.py:443 | 点击结束时间标签的超时（fail-fast） |

### 关键词检测

| 参数 | 值 | 位置 | 说明 |
|------|---|------|------|
| 闪电检测关键词 | 10 个 | booker.py:375-376 | `没有可用时间 没有可约时间 约满 不可预约 当前不可用 无法预约 已满 已被 不可用 没有可用` |
| 轮询检测关键词 | 10 个 | booker.py:387-390 | 同上，顺序略不同 |
| `_kw_display` 映射 | 2 条 | booker.py:369-372 | `不可用→该座位不可用` `没有可用时间→当前的座位已经没有可用时间啦` |
...
| 参数 | 值 | 位置 | 说明 |
|------|---|------|------|
| 验证码图片读取超时 | `1.0` 秒 | booker.py:487 | `_read_captcha_images` 默认超时 |
| 图片读取轮询间隔 | `0.05` 秒 | booker.py:507 | 每 50ms 检查图片是否加载完成 |
| 验证码弹窗等待 | `3` 秒 | booker.py:589 | `pre_solve_captcha` 中等待验证码弹窗容器 |
| 预解码图片读取超时 | `1.2` 秒 | booker.py:601 | 预解码阶段读取验证码图片的超时 |

### 闪电提交（`fire_captcha_blitz`）

| 参数 | 值 | 位置 | 说明 |
|------|---|------|------|
| 确认按钮等待循环 | `60` 次 × `0.05` 秒 = `3` 秒 | booker.py:647-648 | 等待确认按钮可点击 |
| JS 回退点击阈值 | `10` 次（0.5 秒后） | booker.py:650 | ActionChains 失败后切换 JS 点击 |
| 验证码结果超时 | `3.0` 秒 | booker.py:745 | 闪电提交后等待验证结果 |
| 二次提交检查 | `0.3` 秒 | booker.py:758 | 检查提交按钮是否仍可点击 |

### 验证码确认（`_wait_captcha_result`）

| 参数 | 值 | 位置 | 说明 |
|------|---|------|------|
| 轮询超时 | `3.2` 秒 | booker.py:289 | 等待验证码提交反馈的最长时间 |
| 轮询间隔 | `0.06` 秒 | booker.py:289 | 每 60ms 检查一次反馈 |
| page_source 扫描间隔 | `0.5` 秒 | booker.py:317 | UI 选择器无结果时，每 500ms 扫一次 page_source |

### 验证码刷新

| 参数 | 值 | 位置 | 说明 |
|------|---|------|------|
| 刷新后等待超时 | `1.0` 秒 | booker.py:826 | `_refresh_click_captcha` 刷新后等新图片 |
| 关弹窗按钮等待 | `0.1` 秒/个 | booker.py:815 | 每点击一个 message-box 按钮后等 100ms |
...
| 参数 | 值 | 位置 | 说明 |
|------|---|------|------|
| 结果文本等待超时 | `3` 秒 | booker.py:877 | `check_result` 中 WebDriverWait 等待预约结果 |
| 轮询频率 | `0.1` 秒 | booker.py:877 | 预约结果检查的 poll_frequency |
| 座位网格等待超时 | `5.0` 秒 | booker.py:950 | `ensure_on_seat_grid` 等待座位元素 |
| 自选座位按钮等待 | `3` 秒 | booker.py:956 | 等待"自选座位"按钮 |
| 自选座位点击后等待 | `0.3` 秒 | booker.py:959 | 点击后等页面跳转 |
| reserve-box 消失等待 | `1` 秒 | booker.py:1001 | 点击关闭按钮后等弹窗消失 |
| ESC 关弹窗后等待 | `0.3` 秒 | booker.py:1015 | 按 ESC 关弹窗后等 300ms |
...
| YOLO letterbox 填充 | `114` | `114` | yolo_onnx:83 | letterbox 灰色填充 |
| Siamese 裁剪填充 | `128` | `128` | click1:84 | 字符裁剪区域填充值 |
...
### OCR 后处理（`core/captcha.py`）[已废弃]

> [!NOTE]
> 在 v5.0.0 版本中，传统的字符分割 + 简单 OCR 后处理已被 YOLOv4 + Siamese 深度学习点选验证码识别系统完全取代。`core/captcha.py` 目前仅保留最基础的 ddddocr 兼容实现，上述所有参数（如二值化阈值、文本框扩展等）在 v5.0.0 中均已随旧的验证码解析逻辑废弃。
